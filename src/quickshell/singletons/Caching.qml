...
QtObject {
    ...
    readonly property string yoakeDir: Quickshell.env("YOAKE_DIR")
    ...
    readonly property string cacheDir: Quickshell.env("QS_CACHE_DIR") ? Quickshell.env("QS_CACHE_DIR") : (home + "/.cache/yoake")
    readonly property string stateDir: Quickshell.env("QS_STATE_DIR") ? Quickshell.env("QS_STATE_DIR") : (home + "/.local/state/yoake")
    readonly property string runDir: Quickshell.env("QS_RUN_DIR") ? Quickshell.env("QS_RUN_DIR") : ((xdgRuntimeDir !== "" ? xdgRuntimeDir : "/tmp") + "/yoake")
    ...
    function getCacheDir(widgetName) {
        if (!widgetName || widgetName === "yoake" || cacheDir.endsWith("/" + widgetName)) {
    ...
    function getStateDir(widgetName) {
        if (!widgetName || widgetName === "yoake" || stateDir.endsWith("/" + widgetName)) {
    ...
    function getRunDir(widgetName) {
        if (!widgetName || widgetName === "yoake" || runDir.endsWith("/" + widgetName)) {
    ...
    function getLogDir(widgetName) {
        if (!widgetName || widgetName === "yoake" || logDir.endsWith("/" + widgetName)) {
