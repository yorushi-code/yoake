...
Item {
    ...
    property string workspacesStyle: {
        ...
        if (ss && ss.workspacesStyle) return ss.workspacesStyle;
    ...
    property int workspaceCount: {
        let ss = Config.getSetting("sideBar", {});
        if (ss && ss.workspaceCount !== undefined) return ss.workspaceCount;
        let bs = Config.getSetting("bar", {});
        if (bs && bs.sideWorkspaceCount !== undefined) return bs.sideWorkspaceCount;
    ...
    function syncSettings() {
        let ss = Config.getSetting("sideBar", {});
        let bs = Config.getSetting("bar", {});
        if (ss && ss.workspacesStyle) {
            barSideModulesRoot.workspacesStyle = ss.workspacesStyle;
        } else if (bs && bs.sideWorkspacesStyle) {
            barSideModulesRoot.workspacesStyle = bs.sideWorkspacesStyle;
        } else if (bs && bs.workspacesStyle) {
            barSideModulesRoot.workspacesStyle = bs.workspacesStyle;
        } else {
            barSideModulesRoot.workspacesStyle = "pills";
        }

        if (ss && ss.workspaceCount !== undefined) {
            barSideModulesRoot.workspaceCount = ss.workspaceCount;
        } else if (bs && bs.sideWorkspaceCount !== undefined) {
            barSideModulesRoot.workspaceCount = bs.sideWorkspaceCount;
        } else if (bs && bs.workspaceCount !== undefined) {
    ...
    function setWorkspacesStyle(styleName) {
        barSideModulesRoot.workspacesStyle = styleName;
        ...
        currentBar.sideWorkspacesStyle = styleName;
        Config.setSetting("bar", currentBar);
    }

    function setWorkspaceCount(count) {
        barSideModulesRoot.workspaceCount = count;
        let currentSide = Config.getSetting("sideBar", {});
        currentSide.workspaceCount = count;
        Config.setSetting("sideBar", currentSide);
        let currentBar = Config.getSetting("bar", {});
    ...
    Connections {
        ...
        function onSettingsLoaded() {
            barSideModulesRoot.syncSettings();
        }
    }
            ...
            Rectangle {
                ...
                ColumnLayout {
                    ...
                    Rectangle {
                        ...
                        RowLayout {
                            ...
                            NumberSelector {
                                ...
                                stepSize: 1
                                decimals: 0
                                value: barSideModulesRoot.workspaceCount
                                ...
                                onValueChanged: {
                                    ...
                                    if (barSideModulesRoot.workspaceCount !== rounded) {
                                        barSideModulesRoot.setWorkspaceCount(rounded);
                                    }
                                }
                                onTriggered: {
                                    barSideModulesRoot.setWorkspaceCount(Math.round(workspaceCountSelector.value));
                                }
                            }
                        }
                    }
                    ...
                    GridLayout {
                        ...
                        Repeater {
                            model: barSideModulesRoot.workspaceStyles
                            delegate: Rectangle {
                                id: styleCard
                                required property var modelData
                                required property int index

                                readonly property bool isSelected: barSideModulesRoot.workspacesStyle === modelData.id
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        ...
                                        barSideModulesRoot.setWorkspacesStyle(modelData.id);
                                    }
                                }
                                            ...
                                            Loader {
                                                ...
                                                onLoaded: {
                                                    if (item) {
                                                        ...
                                                        item.widget = barSideModulesRoot.previewWidget;
                                                    }
                                                }
                                            }
